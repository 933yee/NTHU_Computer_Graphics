...
enum MagnificationTextureFilteringMode
{
	MagnificationNearest = 0,
	Linear = 1,
};
MagnificationTextureFilteringMode cur_magnification_mode = MagnificationNearest;

enum MinificationTextureFilteringMode
{
	MinificationNearest = 0,
	LinearMipmapLinear = 1,
};
MinificationTextureFilteringMode cur_minification_mode = MinificationNearest;
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	{
		glUniform1i(uniform.PerPixelLightingMode, 1 - per_vertex_or_per_pixel);
		...
		glUniform1i(uniform.IsEye, models[cur_idx].shapes[i].material.isEye);
		if (models[cur_idx].shapes[i].material.isEye == 1)
		{
			glUniform1f(uniform.OffsetX, models[cur_idx].shapes[i].material.offsets[cur_eye_id].x);
			glUniform1f(uniform.OffsetY, models[cur_idx].shapes[i].material.offsets[cur_eye_id].y);
		}
		...
		switch (cur_magnification_mode)
		{
		case MagnificationNearest:
		...
		switch (cur_minification_mode)
		{
		case MinificationNearest:
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_G:
			if (cur_magnification_mode == MagnificationTextureFilteringMode::MagnificationNearest)
				cur_magnification_mode = MagnificationTextureFilteringMode::Linear;
			else
				cur_magnification_mode = MagnificationTextureFilteringMode::MagnificationNearest;
			break;
		case GLFW_KEY_B:
			if (cur_minification_mode == MinificationTextureFilteringMode::MinificationNearest)
				cur_minification_mode = MinificationTextureFilteringMode::LinearMipmapLinear;
			else
				cur_minification_mode = MinificationTextureFilteringMode::MinificationNearest;
			break;
		case GLFW_KEY_RIGHT:
			++cur_eye_id;
			if (cur_eye_id > 6) cur_eye_id = 0;
			break;
		case GLFW_KEY_LEFT:
			--cur_eye_id;
			if (cur_eye_id < 0) cur_eye_id = 6;
			break;
		default:
			break;
		}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case LightEdit:
		...
		case DirectionalLight:
		case PositionLight:
			lightInformation[cur_light_mode].Diffuse += Vector3(yoffset, yoffset, yoffset) / 10.0f;
			break;
		case SpotLight:
			lightInformation[cur_light_mode].Cutoff += yoffset;
	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case TransMode::LightEdit:
				lightInformation[cur_light_mode].Position.x += diff_x / 400.0;
				lightInformation[cur_light_mode].Position.y -= diff_y / 400.0;
				break;
			}
		}
	}
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		{
			material.isEye = 1;
			material.offsets = {{0.0f, 0.0f}, {0.0f, 0.75f}, {0.0f, 0.5f}, {0.0f, 0.25f}, {0.5f, 0.0f}, {0.5f, 0.75f}, {0.5f, 0.5f}, {0.5f, 0.25f}};
		}
		else {
			material.isEye = 0;
		}
...
void setUniformVariables()
{
	...
	// ·Ç³Æ¤@°ï uniform data
	uniform.CameraPosition = glGetUniformLocation(program, "CameraPosition");
	uniform.LightPosition = glGetUniformLocation(program, "LightPosition");
	uniform.LightDirection = glGetUniformLocation(program, "LightDirection");
	...
	uniform.Ambient = glGetUniformLocation(program, "Ambient");
	uniform.Diffuse = glGetUniformLocation(program, "Diffuse");
	uniform.Specular = glGetUniformLocation(program, "Specular");
	uniform.Shininess = glGetUniformLocation(program, "Shininess");
	uniform.Constant = glGetUniformLocation(program, "Constant");
	uniform.Linear = glGetUniformLocation(program, "Linear");
	uniform.Quadratic = glGetUniformLocation(program, "Quadratic");
	uniform.Exponent = glGetUniformLocation(program, "Exponent");
	uniform.Cutoff = glGetUniformLocation(program, "Cutoff");
	uniform.LightMode = glGetUniformLocation(program, "LightMode");
	uniform.PerPixelLightingMode = glGetUniformLocation(program, "PerPixelLightingMode");
	// [TODO] Get uniform location of texture
	uniform.DiffuseTexture = glGetUniformLocation(program, "DiffuseTexture");
	uniform.IsEye = glGetUniformLocation(program, "IsEye");
	uniform.OffsetX = glGetUniformLocation(program, "OffsetX");
	uniform.OffsetY = glGetUniformLocation(program, "OffsetY");
}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "110062222 HW3", NULL, NULL);
    if (window == NULL)
    {
